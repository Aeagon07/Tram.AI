...
def format_area_report(area_label: str, lat: float, lon: float, backend: dict) -> str:
    ...
    # Events
    event_today = event.get("likely_event_today", "")
    if event_today and "no" not in event_today.lower() and "none" not in event_today.lower():
        lines.append("🎪 *EVENTS TODAY*")
        lines.append(f"  {event_today}")
        if event.get("estimated_attendance"):
            lines.append(f"  👥 Expected crowd: ~{event['estimated_attendance']}")
        specific = event.get("specific_details", "")
        if specific:
            lines.append(f"  ℹ️ {specific[:120]}")
        lines.append("")
    else:
...
def format_commute_report(orig_label, orig_lat, orig_lon,
    ...
    if event.get("likely_event_today") and "no" not in event.get("likely_event_today","").lower():
        lines += ["", f"🎪 *Event:* {event['likely_event_today']}"]
        if event.get("estimated_attendance"):
            lines.append(f"  👥 Crowd: ~{event['estimated_attendance']}")
...
def handle_message(sender: str, body: str) -> str:
    ...
    if step == "idle":
        ...
        # Free-form query — handle directly
        if is_free_query(body):
            return handle_free_query(sender, body)

        return "🙏 Say *hi* to start, or ask about any Pune area (e.g. _\"events near Koregaon Park\"_)"
...
def handle_free_query(sender: str, body: str) -> str:
    ...
    backend_data = call_backend_analyze_coords(lat, lon, area_label)
    ...
    return format_area_report(area_label, lat, lon, backend_data)
