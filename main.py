...
# Prefer explicit OPENROUTER_API_KEY but allow OPENAI_API_KEY as a fallback.
openai_api_key = os.getenv("OPENROUTER_API_KEY") or os.getenv("OPENAI_API_KEY")
if not openai_api_key:
    # Try loading .env.example as a last-resort convenience during local development
...
if not openai_api_key:
    logger.error("Missing API key: set OPENROUTER_API_KEY or OPENAI_API_KEY in your environment (.env)")
    raise RuntimeError(
        "Missing API key: set OPENROUTER_API_KEY or OPENAI_API_KEY in your environment (.env)"
    )
...
# Ensure weather key is read before startup checks
OPENWEATHER_API_KEY = os.getenv("OPENWEATHER_API_KEY")

# --- STARTUP ENVIRONMENT CHECKS ---
required_envs = {
    "OPENROUTER_API_KEY or OPENAI_API_KEY": bool(openai_api_key),
...
if missing:
    logger.warning("Missing environment variables detected: %s", ", ".join(missing))
    logger.warning("Some features (Mappls traffic, weather, or OpenRouter calls) may be disabled until these are set.")
else:
...
def call_openrouter_chat(messages, model="google/gemini-2.0-flash-001", temperature=0.2):
    """Direct POST to OpenRouter with explicit Authorization header as a robust fallback.
    Returns parsed JSON response or raises Exception on failure.
    """
...
def scrape_webpage_text(url: str, timeout: int = 10) -> str:
    ...
    try:
        ...
        # return first 1500 chars to avoid prompt bloat
        return text[:1500]
    except Exception as e:
...
def fetch_live_data(venue_name: str) -> str:
    ...
    queries = [
        f"{venue_name} Pune events festival schedule 2026",
        f"{venue_name} Pune traffic current status news",
        f"{venue_name} Pune official website event calendar"
    ]
        ...
        try:
            ...
            for i, el in enumerate(soup.select(".result")):
                ...
                if link_url not in seen_urls:
                    ...
                    # Priority keywords: 'fest', 'event', '2026', 'traffic', 'pune'
                    if any(kw in title_text.lower() or kw in snippet_text.lower() for kw in ["fest", "event", "2026", "news", "official"]):
...
def root():
    return {"status": "ok", "map_dashboard": "/map"}
>>>>>>> origin/feature/final-backend-push
